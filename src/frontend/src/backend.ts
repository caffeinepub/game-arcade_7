...
import type { ScoreEntry as _ScoreEntry, UserProfile as _UserProfile, UserRole as _UserRole } from "./declarations/backend.did.d.ts";
export class Backend implements backendInterface {
    ...
    async getAllHighScores(): Promise<Array<ScoreEntry>> {
        if (this.processError) {
            try {
    ...
    async getGlobalLeaderboard(arg0: string): Promise<Array<ScoreEntry>> {
        if (this.processError) {
            try {
    ...
    async getHighScore(arg0: string): Promise<ScoreEntry | null> {
        if (this.processError) {
            try {
    ...
    async getPlayerCount(arg0: string): Promise<bigint> {
        if (this.processError) {
            try {
                const result = await this.actor.getPlayerCount(arg0);
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
...
function from_candid_opt_n6(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_ScoreEntry]): ScoreEntry | null {
    return value.length === 0 ? null : value[0];
}
